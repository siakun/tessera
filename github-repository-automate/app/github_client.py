...
class GitHubClient:
        ...
        async with httpx.AsyncClient(headers=self.headers, timeout=30) as client:
            ...
            for source in settings.get_accounts():
                if source["type"] == "user":
                    ...
                    tasks.append(self._get_org_repos(client, source["name"]))
            ...
            # 각 리포의 커밋 개수를 동시에 조회
            repo_data_tasks = [
                self._build_repo_data(client, raw, owner)
    ...
    async def get_repo_by_full_name(self, full_name: str) -> RepoData:
        ...
        async with httpx.AsyncClient(headers=self.headers, timeout=30) as client:
            resp = await client.get(f"{API_BASE}/repos/{full_name}")
            resp.raise_for_status()
            raw = resp.json()
            owner = full_name.split("/")[0]
            commit_count = await self._get_commit_count(client, full_name)
    ...
    async def _get_org_repos(self, client: httpx.AsyncClient, org: str) -> list[dict]:
        """조직의 모든 리포지토리를 가져온다."""
        repos = []
        page = 1
        while True:
    ...
    async def _get_commit_count(self, client: httpx.AsyncClient, full_name: str) -> int:
    ...
    async def _build_repo_data(
        self, client: httpx.AsyncClient, raw: dict, owner: str
    ) -> RepoData:
